...
def train_one_epoch(model, optimizer, train_loader_1, train_loader_2, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter_1, dataloader_iter_2, tb_log=None, leave_pbar=False, use_amp=False):
    if rank == 0:
    ...
    if use_amp:
        scaler = torch.cuda.amp.GradScaler(enabled=use_amp, init_scale=2.0**16)
...
def train_model(model, optimizer, train_loader_1, train_loader_2, model_func, lr_scheduler, optim_cfg,
                ...
                max_ckpt_save_num=50, merge_all_iters_to_one_epoch=False, logger=None, use_amp=False):
